...
st.markdown(
    ...
    /* 2.4. KHÓA CỨNG TOÀN CỤC (GLOBAL CSS GRID): ÉP CHIA 2 CỘT NGANG 50-50 BẤT KỂ MÀN HÌNH TO NHỎ */
    
    /* --- Áp dụng cho Form tìm kiếm trạm --- */
    ...
    div[data-testid="stForm"] div[data-testid="stHorizontalBlock"] > div[data-testid="column"] {
        ...
        padding: 0 !important; /* Loại bỏ padding gây tràn viền */
        flex: none !important;
    }

    /* --- Áp dụng cho Khu vực ô nhập Đăng nhập --- */
    .khu-vuc-dang-nhap div[data-testid="stHorizontalBlock"] {
        display: grid !important;
        grid-template-columns: 1fr 1fr !important; /* Khóa chặt 2 ô ngang tỉ lệ 50-50 */
        gap: 8px !important;
        width: 100% !important;
        box-sizing: border-box !important;
    }
    .khu-vuc-dang-nhap div[data-testid="stHorizontalBlock"] > div[data-testid="column"] {
        width: 100% !important;
        max-width: 100% !important;
        min-width: 0 !important;
        padding: 0 !important;
    ...
    /* Đồng bộ kích thước hiển thị của ô nhập liệu trên mọi thiết bị */
    div[data-testid="stForm"] input, .khu-vuc-dang-nhap input {
    ...
    /* 2.5. ĐÁP ỨNG THIẾT BỊ DI ĐỘNG (MOBILE RESPONSIVE) - TỐI ƯU LAYER BẢN ĐỒ */
...
if not st.session_state.logged_in:
    ...
    # Bọc khu vực nhập liệu đăng nhập vào thẻ DIV để ép hạ tầng CSS Grid khóa cứng tỉ lệ 50-50 ngang
    st.markdown('<div class="khu-vuc-dang-nhap">', unsafe_allow_html=True)
    col_login_1, col_login_2 = st.columns(2)
    with col_login_1:
        tai_khoan_nhap = st.text_input("Tài khoản:", value="", key="username_input", placeholder="Tài khoản", label_visibility="collapsed")
    with col_login_2:
        mat_khau_nhap = st.text_input("Mật khẩu:", type="password", key="password_input", placeholder="Mật khẩu", label_visibility="collapsed")
        ...
        with col_left_search:
            with st.expander("🔍 TÌM KIẾM TRẠM", expanded=True):
                with st.form("form_tra_cuu", clear_on_submit=True):
                    # --- Hàng 1: MCC và MNC (Luôn luôn khóa 2 cột ngang 50-50) ---
                    col_f1, col_f2 = st.columns(2)
                    with col_f1:
                        f1 = st.text_input("Mã Quốc gia:", key="mcc_in", placeholder="MCC", label_visibility="collapsed").strip()
                    with col_f2:
                        f2 = st.text_input("Mã Mạng:", key="mnc_in", placeholder="MNC", label_visibility="collapsed").strip()
                    
                    # --- Hàng 2: LAC/TAC và Cell ID (Luôn luôn khóa 2 cột ngang 50-50) ---
                    col_f3, col_f4 = st.columns(2)
                    with col_f3:
                        f3 = st.text_input("Mã Vùng:", key="lac_in", placeholder="LAC/TAC", label_visibility="collapsed").strip()
                    with col_f4:
                        f4 = st.text_input("Cell ID:", key="cell_in", placeholder="Cell ID", label_visibility="collapsed").strip()
